 ...
 *
 * Skeleton is built from Three.js primitives (bone-coloured cylinders, spheres,
 * boxes). Rapier capsule physics body with enemy collision groups so skeletons
 * don't push each other into a pile.
 *
...
export class Enemy {
  ...
  constructor(
    ...
    this.torsoGroup.add(this.mkMesh(
      new THREE.CylinderGeometry(0.13, 0.15, 0.46, 7), MAT_BONE,
    ));

    // Spine connecting ribcage to hips
    const spine = this.mkMesh(new THREE.CylinderGeometry(0.034, 0.038, 0.5, 5), MAT_BONE);
    spine.position.set(0, -0.28, 0);
    this.torsoGroup.add(spine);

    // Hip bone
    const hip = this.mkMesh(new THREE.BoxGeometry(0.3, 0.09, 0.14), MAT_BONE);
    ...
    const skull = this.mkMesh(new THREE.SphereGeometry(0.15, 8, 6), MAT_BONE);
    skull.scale.set(1, 1.08, 0.92);
    this.headGroup.add(skull);

    const jaw = this.mkMesh(new THREE.BoxGeometry(0.12, 0.06, 0.09), MAT_BONE);
    jaw.position.set(0, -0.12, 0.05);
    this.headGroup.add(jaw);

    // Glowing eye sockets
    const eyeL = this.mkMesh(new THREE.SphereGeometry(0.036, 5, 4), MAT_EYE);
    eyeL.position.set(-0.058, 0.022, 0.11);
    this.headGroup.add(eyeL);

    const eyeR = this.mkMesh(new THREE.SphereGeometry(0.036, 5, 4), MAT_EYE);
    ...
    const lUpper = this.mkMesh(new THREE.CylinderGeometry(0.035, 0.031, 0.36, 5), MAT_BONE);
    lUpper.position.set(0, -0.18, 0);
    this.leftArmGroup.add(lUpper);

    const lElbow = this.mkMesh(new THREE.SphereGeometry(0.042, 5, 4), MAT_JOINT);
    lElbow.position.set(0, -0.36, 0);
    this.leftArmGroup.add(lElbow);

    const lLower = this.mkMesh(new THREE.CylinderGeometry(0.027, 0.023, 0.32, 5), MAT_BONE);
    ...
    const rUpper = this.mkMesh(new THREE.CylinderGeometry(0.035, 0.031, 0.36, 5), MAT_BONE);
    rUpper.position.set(0, -0.18, 0);
    this.rightArmGroup.add(rUpper);

    const rElbow = this.mkMesh(new THREE.SphereGeometry(0.042, 5, 4), MAT_JOINT);
    rElbow.position.set(0, -0.36, 0);
    this.rightArmGroup.add(rElbow);

    const rLower = this.mkMesh(new THREE.CylinderGeometry(0.027, 0.023, 0.32, 5), MAT_BONE);
    rLower.position.set(0, -0.52, 0);
    this.rightArmGroup.add(rLower);

    this.torsoGroup.add(this.rightArmGroup);

    // ── Weapon (crude club/bone sword on right arm) ────────────────────────
    this.weaponGroup = new THREE.Group();
    this.weaponGroup.position.set(0.04, -0.68, 0);
    this.weaponGroup.rotation.z = 0.12;

    const clubHead = this.mkMesh(new THREE.BoxGeometry(0.068, 0.7, 0.057), MAT_WEAPON);
    clubHead.position.set(0, 0.35, 0);
    this.weaponGroup.add(clubHead);

    const clubHandle = this.mkMesh(new THREE.CylinderGeometry(0.024, 0.02, 0.28, 5), MAT_JOINT);
    ...
    const ltThigh = this.mkMesh(new THREE.CylinderGeometry(0.046, 0.042, 0.38, 5), MAT_BONE);
    ltThigh.position.set(0, -0.19, 0);
    this.leftLegGroup.add(ltThigh);

    const ltKnee = this.mkMesh(new THREE.SphereGeometry(0.054, 5, 4), MAT_JOINT);
    ltKnee.position.set(0, -0.38, 0);
    this.leftLegGroup.add(ltKnee);

    const ltShin = this.mkMesh(new THREE.CylinderGeometry(0.036, 0.032, 0.34, 5), MAT_BONE);
    ...
    const rtThigh = this.mkMesh(new THREE.CylinderGeometry(0.046, 0.042, 0.38, 5), MAT_BONE);
    rtThigh.position.set(0, -0.19, 0);
    this.rightLegGroup.add(rtThigh);

    const rtKnee = this.mkMesh(new THREE.SphereGeometry(0.054, 5, 4), MAT_JOINT);
    rtKnee.position.set(0, -0.38, 0);
    this.rightLegGroup.add(rtKnee);

    const rtShin = this.mkMesh(new THREE.CylinderGeometry(0.036, 0.032, 0.34, 5), MAT_BONE);
  ...
  update(delta: number, playerPos: THREE.Vector3): void {
    ...
    switch (this.aiState) {
      ...
      case EnemyAIState.ATTACK_WINDUP:
        ...
        this.animAttackWindup(Math.min(this.stateTimer / 0.5, 1));
        break;
